...
SELECT * FROM animes WHERE titulo LIKE "F%"; -- no tiene encuenta las mayúsculas o minúsculas que empiezen en f
SELECT * FROM animes WHERE titulo LIKE "%n"; -- no tiene encuenta las mayúsculas o minúsculas que terminen en n
SELECT * FROM animes WHERE titulo LIKE "%a%"; -- que contenga una a en cualquier sitio UN BUSCADOR SIEMPRE CON LIKE

SELECT * FROM animes;
SELECT * FROM estudios;
-- Mostrar el nombre de anime, estudios y la ciudad
...
-- Agregando campos a estudios y animes
SELECT * FROM animes;
...
-- Recordando el update y actualizando el numero de num_temporadas
UPDATE animes 
...
-- Con el ID automatico para que lo inserte
INSERT INTO 
	animes (titulo, nombre_estudio, anno_estreno, num_temporadas)
	VALUES("Black Clover", "Pierrot", 2017, 1);
SELECT * FROM animes;
-- Eso significa que todo los cambios que se han realizado TENEMOS QUE CONFIRMARLOS A 0
-- COMMIT => CREA UN PUNTO DE GUARDADO
SET AUTOCOMMIT = 0; -- QUITA EL AUTO GUARDADO
INSERT INTO animes VALUES(26, "Kimetsu no blabla", "Pierrot", 2020, 3);
ROLLBACK; -- BORRA TODO HASTA EL ULTIMO "PUNTO DE GUARDADO"
DELETE FROM animes;
SELECT * FROM animes;
SET sql_safe_updates = 0; -- Eso para desactivar el modo niño y que pueda hacer un borrado masivo

-- VIENDO COMO FUNCIONA EL ROLLBACK Y EL COMMIT 
INSERT INTO animes VALUES(26, "Kimetsu no Yaiba", "Pierrot", 2020, 3);
COMMIT; -- GUARDO MANUALMENTE
ROLLBACK;
SELECT * FROM animes;
